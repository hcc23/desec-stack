...
class Domain(ExportModelOperationsMixin('Domain'), models.Model):
    ...
    def is_covered_by_foreign_zone(self):
        ...
        private_components += [self.public_suffix]
        ...
        return Domain.objects.filter(Q(name__in=private_domains) & ~Q(owner=self.owner_or_none)).exists()
    ...
    def covers_foreign_zone(self):
        ...
        return Domain.objects.filter(Q(name__endswith=f'.{self.name}') & ~Q(owner=self.owner_or_none)).exists()
